...
const createTransporter = () => {
  // Nếu không có email config, return null (fallback to console log)
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: process.env.EMAIL_USER, // Email của bạn (ví dụ: yourname@gmail.com)
        pass: process.env.EMAIL_PASSWORD, // App Password (16 ký tự từ Google)
      },
    });

    // Verify connection configuration (async, không block - chỉ log)
...
export const sendVerificationEmail = async ({
  ...
  try {
    ...
    // Nếu không có email config, throw error (không fallback DEV mode)
    if (!transporter) {
    ...
    console.log(`📧 Email subject: Verify Your Email - TicketVibe`);
    ...
    // Không fallback - throw error để caller biết email không được gửi
    throw error;
  }
};
...
export const sendResetPasswordEmail = async ({
  ...
  try {
    ...
    console.log(`📧 Transporter created successfully, sending reset password email...`);
    ...
    console.log(`📧 Message ID: ${info.messageId}`);
    console.log(`📧 Email subject: Reset Your Password - TicketVibe`);
    console.log(`📧 From: ${process.env.EMAIL_USER}`);
    console.log(`📧 To: ${to}`);
    return true;
  } catch (error: any) {
