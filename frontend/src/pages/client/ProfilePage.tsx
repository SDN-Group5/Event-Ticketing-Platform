...
export const ProfilePage: React.FC = () => {
    ...
    const handleSaveProfile = async () => {
        ...
        if (!fnTrimmed || fnTrimmed.length < 2 || fnTrimmed.length > 50 || !nameRegex.test(fnTrimmed)) {
            setSaveMsg({ ok: false, text: 'Họ không hợp lệ (2-50 ký tự, chỉ chữ cái, khoảng trắng, dấu gạch ngang)' });
            setSaving(false);
            return;
        }
        if (!lnTrimmed || lnTrimmed.length < 2 || lnTrimmed.length > 50 || !nameRegex.test(lnTrimmed)) {
            setSaveMsg({ ok: false, text: 'Tên không hợp lệ (2-50 ký tự, chỉ chữ cái, khoảng trắng, dấu gạch ngang)' });
            setSaving(false);
            return;
        }
                                ...
                                <div className="grid gap-4 max-w-2xl bg-white/5 border border-white/10 rounded-xl p-5">
                                    <div className="grid sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input className={inputCls} maxLength={50} value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} />
                                        </div>
                                        <div>
                                            <label className={labelCls}>Tên</label>
                                            <input className={inputCls} maxLength={50} value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} />
                                        </div>
                                    </div>
                                    <div>
